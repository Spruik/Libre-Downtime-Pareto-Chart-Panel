      ...
      myChart.on('click', (p) => {
        if (p.seriesType === 'bar' && p.seriesName !== 'Reasons') {
          ...
          if (!pie.checkIsDurationMode()) {
            ...
            // get reason label
            const sortedReasonsLabel = dp.filterItems(sortedReasons, 'name')
            // get reason value
            const sortedReasonsValue = dp.filterItems(sortedReasons, 'value')
            // get reason percent
            ...
            // get total value
            const totalVal = sortedReasonsValue.reduce((total, val) => total + val)
            ...
            const sortedReasons = dp.sortMax(reasons, 'duration')
            const sortedReasonsLabel = dp.filterItems(sortedReasons, 'name')
            const sortedReasonsValue = dp.filterItems(sortedReasons, 'duration')
...
ChartCtrl.templateUrl = 'public/plugins/libre-downtime-pareto-chart-panel/partials/module.html'
